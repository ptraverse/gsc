...
from calculate import MSG_SUCCESS as R2C_SUCCESS
from calculate import MSG_FAIL as R2C_FAIL
...
class R2CStatusCheckerCls: #{
  ...
  def CheckR2CStatus(self): #{
    LogMsg(self, "Checking read-to-contig jobs status...\n%s" %
    ...
    if (0 == self.num_jobs['total']): #{
      raise R2CStatusCheckerError("could not find any jobs in \"%s\"" %
        self.options.jobs_dir)
    #} end if
  ...
  def GetJobPaths(self): #{
    ...
    job_path_template = os.path.join(self.options.jobs_dir, "*-r2c.*.sh")
    DebugMsg(self, "TEMPLATE: %s" % job_path_template)
  ...
  def ResubmitJobs(self): #{
    ...
    if (len(self.unfinished_jobs) != num_unfinished_jobs): #{
      raise R2CStatusCheckerError("wrong number of unfinished jobs in list")
    #} end if
  ...
  def ResubmitLocal(self, job): #{
    ...
    #} end if
    if (0 > return_code): #{
      raise R2CStatusCheckerError("Resubmission command was terminated "
        "by signal %i" % return_code)
    elif (0 < return_code):
      raise R2CStatusCheckerError("Error running resubmission "
  ...
  def OutputR2CStatus(self): #{
    ...
    #} end for
    LogMsg(self, "R2C status: %s" % self.status)
  #} end def
#} end class
...
class R2CJobCls: #{
  def __init__(self, job_path): #{
    ...
    job_match = re.search(r"^(?P<lib>.*)-r2c\.(?P<num>\d+)\.sh$", file_name)
    if (None == job_match): #{
      self.num = -1
    else:
  ...
  def GetJobLine(self): #{
    ...
    for line in job_file: #{
      job_line = line
    #} end for
    if (None == job_line or "read_to_contig/calculate" not in line): #{
      raise R2CStatusCheckerError \
  ...
  def NewOutPath(self): #{
    if (None == self.output_path): #{
      ...
      if (None == old_out_match): #{
        raise R2CStatusCheckerError("error getting new stdout path for job, "
...
#### EXCEPTION CLASSES ####
class R2CStatusCheckerError(MyError): #{
  pass
#} end class
...
def CheckPaths(options): #{
  path_errors = list()
  CheckDirPath(options.jobs_dir, "read-to-contig cluster jobs", path_errors)
...
def Main(): #{
  ...
  if (parser.num_args == len(args)): #{
    ...
    if (CheckPaths(options)): #{
      try:
        r2c_status_checker = R2CStatusCheckerCls(options)
        WriteCommand(r2c_status_checker, sys.argv)
        r2c_status_checker.CheckR2CStatus()
        r2c_status_checker.OutputR2CStatus()
        if (options.resubmit): #{
          if (0 < len(r2c_status_checker.unfinished_jobs)): #{
            r2c_status_checker.ResubmitJobs()
          else:
            LogMsg(r2c_status_checker,
              "All jobs are complete: nothing to resubmit")
          #} end def
        #} end if
      except (MyError), e:
        ErrMsg("ERROR while checking read-to-contig support status:\n"
               "  %s" % e)
        return ES_RUN_ERR
      # end try
    else:
    ...
    parser.error("you must specify a path to a directory containing "
      "read-to-contig cluster jobs for that library (JOBS_DIR).")
    return ES_OPT_ERR
  #} end if
  return ES_SUCCESS
#} end def
